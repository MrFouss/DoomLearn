...
# save_dir = "/home/esia/Documents/doom-learn/DoomLearn/trainings/training_deadly_corridor"
...
def save_simulation_parameters():
    ...
    with open(parameters_savefile, 'w') as file:
        file.write('screen and game variable learning' + '\n')
        file.write('game variable number=' + str(game_variable_nbr) + '\n')
...
def getGameVariables():
    gvars = np.zeros(1, dtype=np.float32)
    gvars[0] = game.get_game_variable(game.get_available_game_variables()[0])
    return gvars
...
class ReplayMemory:
    def __init__(self, capacity):
        ...
        self.game_vars1 = np.zeros([capacity, game_variable_nbr], dtype=np.float32)
        self.game_vars2 = np.zeros([capacity, game_variable_nbr], dtype=np.float32)
    ...
    def add_transition(self, s1, action, s2, isterminal, reward, game_vars1, game_vars2):
        self.s1[self.pos, :, :, :] = s1
        self.a[self.pos] = action
        if not isterminal:
            self.s2[self.pos, :, :, :] = s2
            self.game_vars2[self.pos, :] = game_vars2
        self.isterminal[self.pos] = isterminal
        self.r[self.pos] = reward
        self.game_vars1[self.pos, :] = game_vars1
    ...
    def get_sample(self, sample_size):
        ...
        return self.s1[i], self.a[i], self.s2[i], self.isterminal[i], self.r[i], self.game_vars1[i], self.game_vars2[i]
...
def create_network(session, available_actions_count):
    ...
    game_var_ = tf.placeholder(tf.float32, [None] + [game_variable_nbr], name="GameVariables")
    ...
    concat_screen_var = tf.concat([conv3_flat, game_var_], 1)
    fc1 = tf.contrib.layers.fully_connected(concat_screen_var, 
                                            num_outputs=512,                  
                                            activation_fn=tf.nn.relu,
                                            weights_initializer=tf.contrib.layers.xavier_initializer(),
                                            biases_initializer=tf.constant_initializer(0.1))
    ...
    def function_learn(s1, target_q, game_var):
        feed_dict = {s1_: s1, target_q_: target_q, game_var_ : game_var}
        l, _, m = session.run([loss, train_step, merged], feed_dict=feed_dict)
        train_writer.add_summary(m, train_steps_since_start)
        return l

    def function_get_q_values(state, game_var):
        return session.run(q, feed_dict={s1_: state, game_var_: game_var})

    def function_get_best_action(state, game_var):
        return session.run(best_a, feed_dict={s1_: state, game_var_: game_var})

    def function_simple_get_best_action(state, game_var):
        return function_get_best_action(state.reshape([1, resolution[0], resolution[1], screen_channels]), game_var.reshape([1, game_variable_nbr]))[0]
...
def learn_from_memory():
    ...
    if memory.size > batch_size:
        s1, a, s2, isterminal, r, game_vars1, game_vars2 = memory.get_sample(batch_size)

        q2 = np.max(get_q_values(s2, game_vars1), axis=1)
        target_q = get_q_values(s1, game_vars2)
        # target differs from q only for the selected action. The following means:
        # target_Q(s,a) = r + gamma * max Q(s2,_) if isterminal else r
        target_q[np.arange(target_q.shape[0]), a] = r + discount_factor * (1 - isterminal) * q2
        learn(s1, target_q, game_vars1)
...
def perform_learning_step(epoch):
    ...
    if random() <= eps:
        ...
        a = get_best_action(s1, game_vars1)
    reward = game.make_action(actions[a], frame_repeat)

    isterminal = game.is_episode_finished()
    s2 = preprocess(game.get_state().screen_buffer) if not isterminal else None
    game_vars2 = getGameVariables() if not isterminal else None

    # Remember the transition that was just experienced.
    memory.add_transition(s1, a, s2, isterminal, reward, game_vars1, game_vars2)
...
if __name__ == '__main__':
    ...
    if not skip_learning:
        ...
        trainingHealthSummary = tf.Summary(value=[tf.Summary.Value(tag="health/training value", simple_value=0)])
        testHealthSummary = tf.Summary(value=[tf.Summary.Value(tag="health/test value", simple_value=0)])
        ...
        for epoch in range(epochs):
            ...
            for learning_step in trange(learning_steps_per_epoch, leave=False):
                ...
                trainingHealthSummary.value[0].simple_value = getGameVariables()[0]
                train_writer.add_summary(trainingHealthSummary, train_steps_since_start)
            ...
            for test_episode in trange(test_episodes_per_epoch, leave=False):
                ...
                while not game.is_episode_finished():
                    ...
                    game_vars = getGameVariables()
                    best_action_index = get_best_action(state, game_vars)

                    game.make_action(actions[best_action_index], frame_repeat)
                    train_steps_since_start += 1
                    trainOrTestSummary.value[0].simple_value = -1
                    testHealthSummary.value[0].simple_value = getGameVariables()[0]
                    train_writer.add_summary(testHealthSummary, train_steps_since_start-1)
